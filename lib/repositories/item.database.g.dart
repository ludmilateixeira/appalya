    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Produto` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `meaning` TEXT)');
  ...
  _$ItemDao(this.database, this.changeListener)
      : _queryAdapter = QueryAdapter(database),
        _produtoInsertionAdapter = InsertionAdapter(
            ...
            'Produto',
            (Produto item) =>
                <String, dynamic>{'id': item.id, 'meaning': item.meaning}),
  ...
  Future<List<Produto>> getAll() async {
    return _queryAdapter.queryList('SELECT * FROM Produto',
        mapper: (Map<String, dynamic> row) =>
            Produto(id: row['id'] as int, meaning: row['meaning'] as String));
  }
